    ...
    <div class="restore-game">
      ...
      <div class="scene-area">
        ...
        <Transition name="fade">
          <div v-if="transformLevel >= 2" class="visual-improvement" style="right: 12%; top: 38%;">🌸</div>
        </Transition>
        <Transition name="fade">
          <div v-if="transformLevel >= 3" class="visual-improvement" style="left: 35%; top: 12%; animation-delay: 1s;">🦋</div>
        </Transition>
        <Transition name="fade">
          <div v-if="transformLevel >= 5" class="visual-improvement" style="right: 20%; bottom: 20%; animation-delay: 0.5s;">👨‍👩‍👧</div>
        </Transition>
      </div>

      <!-- Feedback -->
      <Transition name="fade">
        <div v-if="feedback" class="restore-feedback game-feedback" :class="feedback.ok ? 'fb--ok game-feedback--ok' : 'fb--no game-feedback--no'">
          {{ feedback.message }}
        </div>
      </Transition>
...
const zonesData: PlaceZone[] = [
  { id: 'z1', problem: 'Esquina oscura', problemEmoji: '🌑', accepts: 'lamp', x: 12, y: 50, filled: false, placedEmoji: null,
    successMsg: 'Una calle iluminada es más segura y se siente más viva de noche.' },
  { id: 'z2', problem: 'Pared vacía y gris', problemEmoji: '🧱', accepts: 'mural', x: 68, y: 20, filled: false, placedEmoji: null,
    successMsg: 'Un mural hecho por la comunidad le da identidad al barrio y ahuyenta el vandalismo.' },
  { id: 'z3', problem: 'Nadie se detiene aquí', problemEmoji: '🚶', accepts: 'bench', x: 40, y: 58, filled: false, placedEmoji: null,
    successMsg: 'Una banca invita a la gente a quedarse, platicar y cuidar el espacio.' },
  { id: 'z4', problem: 'Basura en el piso', problemEmoji: '🛍️', accepts: 'bin', x: 58, y: 65, filled: false, placedEmoji: null,
    successMsg: 'Con un bote cerca, la gente tiene dónde tirar su basura correctamente.' },
  { id: 'z5', problem: 'Sol directo, sin sombra', problemEmoji: '☀️', accepts: 'planter', x: 25, y: 30, filled: false, placedEmoji: null,
...
const itemsData: RestoreItem[] = [
  { id: 'i1', name: 'Banca', emoji: '🪑', type: 'bench', used: false, rejected: false, isDistractor: false, distractorMsg: '' },
  { id: 'i2', name: 'Jardinera', emoji: '🪴', type: 'planter', used: false, rejected: false, isDistractor: false, distractorMsg: '' },
  { id: 'i3', name: 'Mural', emoji: '🎨', type: 'mural', used: false, rejected: false, isDistractor: false, distractorMsg: '' },
  { id: 'i4', name: 'Contenedor', emoji: '🗑️', type: 'bin', used: false, rejected: false, isDistractor: false, distractorMsg: '' },
  { id: 'i5', name: 'Luminaria solar', emoji: '💡', type: 'lamp', used: false, rejected: false, isDistractor: false, distractorMsg: '' },
  { id: 'i6', name: 'Carpa de circo', emoji: '🎪', type: 'circus', used: false, rejected: false, isDistractor: true,
    distractorMsg: 'Una carpa no resuelve un problema permanente de la calle.' },
  { id: 'i7', name: 'Alberca inflable', emoji: '🏊', type: 'pool', used: false, rejected: false, isDistractor: true,
    distractorMsg: 'La calle necesita soluciones urbanas, no diversión temporal.' },
]
...
.zone-problem-icon { font-size: 22px; }
.zone-problem { font-size: 9px; font-weight: 800; color: var(--color-text); text-align: center; line-height: 1.2; }
