<template>
  <MinigameShell
    ...
    @timeout="onTimeout"
  >
    <div class="cleanup-game" @pointermove="onPointerMove" @pointerup="onPointerUp">
      <!-- Background -->
      <SceneSky variant="hot" />
      <SceneStreet variant="dirty" />

      <!-- Hint -->
      <div class="cleanup-hint" :class="{ 'hint--active': !!selectedItem }">
        {{ selectedItem ? '⬇️ Arrastra al contenedor o tócalo' : '👆 Toca o arrastra un residuo' }}
      </div>

      <!-- Trash items floating above the scene -->
      <div class="trash-area" ref="trashAreaRef">
        <div
          v-for="item in visibleItems"
          :key="item.id"
          class="trash-item game-item"
          :class="{
            'trash-item--selected game-item--selected': selectedItem?.id === item.id,
            'trash-item--dragging game-item--dragging': dragging?.id === item.id,
          }"
      ...
      <!-- Bins - fixed at bottom, ABOVE the street -->
      <div class="bins-area">
        <div
          v-for="bin in bins"
          :key="bin.id"
...
// Items con nombres específicos que dejan claro su material
const trashItems: TrashItem[] = [
  { id: 't1', name: 'Cáscara de plátano', emoji: '🍌', category: 'organic', x: 10, y: 12, sorted: false },
  { id: 't2', name: 'Botella de plástico', emoji: '🧴', category: 'plastic', x: 52, y: 8, sorted: false },
  { id: 't3', name: 'Periódico viejo', emoji: '📰', category: 'paper', x: 28, y: 30, sorted: false },
  { id: 't4', name: 'Bolsa de plástico', emoji: '🛍️', category: 'plastic', x: 70, y: 22, sorted: false },
  { id: 't5', name: 'Corazón de manzana', emoji: '🍎', category: 'organic', x: 18, y: 50, sorted: false },
  { id: 't6', name: 'Caja de cartón', emoji: '📦', category: 'paper', x: 58, y: 42, sorted: false },
  { id: 't7', name: 'Pañal usado', emoji: '🩲', category: 'non-recyclable', x: 40, y: 55, sorted: false },
  { id: 't8', name: 'Lata de aluminio', emoji: '🥫', category: 'plastic', x: 76, y: 45, sorted: false },
  { id: 't9', name: 'Hojas secas', emoji: '🍂', category: 'organic', x: 45, y: 18, sorted: false },
  { id: 't10', name: 'Vaso de unicel', emoji: '🥤', category: 'non-recyclable', x: 82, y: 35, sorted: false },
]
...
// Drag state
const dragging = ref<TrashItem | null>(null)
const dragPos = ref({ x: 0, y: 0 })
const dragStarted = ref(false)
let rafPending = false
const hoveredBin = ref<string | null>(null)
const trashAreaRef = ref<HTMLElement | null>(null)

const bins = ref<Bin[]>([
  { id: 'organic', label: 'Orgánico', emoji: '🌿', color: '#16a34a', colorLight: '#dcfce7', count: 0, target: 3 },
  { id: 'plastic', label: 'Reciclable', emoji: '♻️', color: '#2563eb', colorLight: '#dbeafe', count: 0, target: 3 },
  { id: 'paper', label: 'Papel/Cartón', emoji: '📄', color: '#d97706', colorLight: '#fef3c7', count: 0, target: 2 },
  { id: 'non-recyclable', label: 'No recicl.', emoji: '🚫', color: '#dc2626', colorLight: '#fee2e2', count: 0, target: 2 },
])

function categoryLabel(category: string): string {
  const map: Record<string, string> = { organic: 'Orgánico', plastic: 'Reciclable', paper: 'Papel/Cartón', 'non-recyclable': 'No reciclable' }
  return map[category] ?? '?'
}
...
// --- Drag support ---
function onPointerDown(item: TrashItem, e: PointerEvent) {
...
// --- Tap support ---
function onItemClick(item: TrashItem) {
...
// --- Shared ---
function itemStyle(item: TrashItem) {
...
function checkSort(item: TrashItem, binId: string) {
  if (item.category === binId) {
    ...
    if (isComplete.value) {
      const gameEl = document.querySelector('.cleanup-game')
      if (gameEl) confettiBurst(gameEl, 24)
      setTimeout(() => { showResult.value = true }, 800)
    }
  } else {
    const correctBin = categoryLabel(item.category)
    showFeedback(`Ese residuo no va en ese contenedor. 💡 Fíjate en el material y el color del bote correcto.`, 'wrong')
...
/* Items area - above the street scene */
.trash-area {
  flex: 1;
  position: relative;
...
.trash-item__emoji { font-size: 28px; filter: drop-shadow(0 1px 2px rgba(0,0,0,0.15)); }
.trash-item__name { font-size: 10px; font-weight: 800; color: var(--color-text); line-height: 1.2; text-align: center; max-width: 70px; }

/* Bins - fixed at bottom, high z-index */
.bins-area {
  display: flex;
  gap: 6px;
